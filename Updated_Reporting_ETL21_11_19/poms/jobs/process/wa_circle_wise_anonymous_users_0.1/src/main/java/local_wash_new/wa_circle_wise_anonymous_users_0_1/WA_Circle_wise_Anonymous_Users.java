...
package local_wash_new.wa_circle_wise_anonymous_users_0_1;
...
public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
    final static byte[] commonByteArrayLock_LOCAL_WASH_NEW_WA_Circle_wise_Anonymous_Users = new byte[0];
    static byte[] commonByteArray_LOCAL_WASH_NEW_WA_Circle_wise_Anonymous_Users = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_LOCAL_WASH_NEW_WA_Circle_wise_Anonymous_Users.length) {
				if(length < 1024 && commonByteArray_LOCAL_WASH_NEW_WA_Circle_wise_Anonymous_Users.length == 0) {
   					commonByteArray_LOCAL_WASH_NEW_WA_Circle_wise_Anonymous_Users = new byte[1024];
				} else {
   					commonByteArray_LOCAL_WASH_NEW_WA_Circle_wise_Anonymous_Users = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_LOCAL_WASH_NEW_WA_Circle_wise_Anonymous_Users, 0, length);
			strReturn = new String(commonByteArray_LOCAL_WASH_NEW_WA_Circle_wise_Anonymous_Users, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_LOCAL_WASH_NEW_WA_Circle_wise_Anonymous_Users) {
 ...
 */
	...
	final String decryptedPassword_tMysqlInput_1 = routines.system.PasswordEncryptUtil.decryptPassword("ENC:[HhQbJD6fq21DSuvC4ZZDYHBM+UoIM7coXyMB5bEWGgj1qR8o]");
...
java.sql.Connection conn_tMysqlOutput_2 = null;
	...
	final String decryptedPassword_tMysqlOutput_2 = routines.system.PasswordEncryptUtil.decryptPassword("ENC:[S0LAT7XTd0dTXNjm9XKeBw3lPsbEcKnHfC0ijYbpBDL70c7q]");
...
int count_tMysqlOutput_2=0;
	    ...
	    String insert_tMysqlOutput_2 = "INSERT INTO `" + "WA_circle_wise_anonymous_line_listing" + "` (`circle_name`,`id`,`mobile_number`,`last_call_end_date`,`last_call_end_time`,`no_of_attempts`,`SMS_reference_no`,`SMS_sent`,`course_start_date`,`first_completion_date`,`operator`,`total_minutes_used`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
	    ...
	    String update_tMysqlOutput_2 = "UPDATE `" + "WA_circle_wise_anonymous_line_listing" + "` SET `circle_name` = ?,`last_call_end_date` = ?,`last_call_end_time` = ?,`no_of_attempts` = ?,`SMS_reference_no` = ?,`SMS_sent` = ?,`course_start_date` = ?,`first_completion_date` = ?,`operator` = ?,`total_minutes_used` = ? WHERE `id` = ? AND `mobile_number` = ?";
 ...
 */
	...
	final String decryptedPassword_tMysqlInput_3 = routines.system.PasswordEncryptUtil.decryptPassword("ENC:[N4LGqSD6tPfB6yGiOuAKgq1lv+vfViR36+R7iHCMVDyDyDxT]");
...
+"sent_notification,d.operator,a.last_call_date,a.last_call_time,a.course_start_date\n\nfrom (select cdrs.circle as 'circle_"
+"name',cdrs.callingNumber as 'mobile_number',\nccf.first_completion,\nccf.sent_notification,\nwccr.smsReferenceNumber,\nDate("
+"max(cdrs.callEndTime)) as 'last_call_date', \nmax(cdrs.callEndTime) as 'last_call_time',\nmin(cdrs.callStartTime) as 'cour"
+"se_start_date'\nfrom wash_swachgrahi_cdrs cdrs \nLEFT join wash_swachchagrahi s on s.id = cdrs.swachchagrahi_id_OID\njoin w"
+"ash_circles c on cdrs.circle = c.name\nleft join WAReporting.WA_course_completion_first ccf on ccf.mobile_number = cdrs.c"
+"allingNumber\nleft join wash_course_completion_records wccr on wccr.creationDate = ccf.first_completion\nwhere s.courseSta"
+"tus = 'ANONYMOUS' \nand cdrs.modificationDate >'"+(String)globalMap.get("lastEtlTime")+"'\ngroup by cdrs.callingNumber,cdrs.circle,ccf.first_completion,\nccf.sent_notification,wccr.smsReferenceNumber,\ncdrs.cal"
+"lingNumber\norder by cdrs.callingNumber) as a\n\nleft join \n(SELECT callingNumber as 'mobile_number',count(*) as 'num_of_at"
+"tempts'\nFROM motech_data_services.wash_swachgrahi_cdrs group by mobile_number\n) as b on a.mobile_number = b.mobile_numbe"
+"r\n\nleft join\n(SELECT mobile_number,sum(duration)/60 as 'duration' \n FROM WAReporting.WA_call_detail_measure group by mob"
+"ile_number) as c \n on c.mobile_number= a.mobile_number\n \nleft join \n(select callingNumber as 'mobile_number', operator,c"
...
pstmtUpdate_tMysqlOutput_2.setNull(10, java.sql.Types.VARCHAR);
                        ...
                        pstmtUpdate_tMysqlOutput_2.setLong(11 + count_tMysqlOutput_2, copyOfMa_course_completion_data.id);

                        pstmtUpdate_tMysqlOutput_2.setLong(12 + count_tMysqlOutput_2, copyOfMa_course_completion_data.mobile_number);
...
public void tMysqlRow_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 final boolean execStat = this.execStat;
	...
	final String decryptedPassword_tMysqlRow_1 = routines.system.PasswordEncryptUtil.decryptPassword("ENC:[QWCWp56ED7UJt9LEl0joFnGsrCH8nWu/OD9Cp84jVwKEUVP/]");
 ...
 */
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = WA_Circle_wise_Anonymous_Users.class.getClassLoader().getResourceAsStream("local_wash_new/wa_circle_wise_anonymous_users_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
...
/************************************************************************************************
 *     113924 characters generated by Talend Open Studio for Data Integration 
 *     on the 21 November, 2019 1:39:24 PM IST
 ************************************************************************************************/