...
//        List<Swachchagrahi> swachchagrahisList = new ArrayList<>();
//
//        if(locationType.equalsIgnoreCase("State")){
//            criteria.add(Restrictions.lt("state",locationId));
//            swachchagrahisList = criteria.list();
//        }
...
//        fromDate = dateAdder(fromDate,1);
        if(locationType.equalsIgnoreCase("state")) {
            Query query = getSession().createSQLQuery("select count(*) from swachchagrahi f  " +
                    "where f.swc_id not in  (select distinct m.swc_id  from wa_course_completion m  where m.has_passed = 1 and m.creationDate < :fromDate)  " +
