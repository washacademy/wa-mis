...
package local_wash_new.swc_sim_changes_test_0_1;
...
/**
 * Job: SWC_sim_changes_test Purpose: to import changed msisdn numbers of
 * swachchagrahi<br>
 * Description: track the MSISDN numbers of the swachchagrahi to aid the audit
 * of contact numbers <br>
 * 
 * @author user@talend.com
 * @version 7.3.1.20191031_1204-M3
 * @status
 */
public class SWC_sim_changes_test implements TalendJob {
	...
	private final String jobName = "SWC_sim_changes_test";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					SWC_sim_changes_test.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(SWC_sim_changes_test.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row3Struct implements routines.system.IPersistableRow<row3Struct> {
		final static byte[] commonByteArrayLock_LOCAL_WASH_NEW_SWC_sim_changes_test = new byte[0];
		static byte[] commonByteArray_LOCAL_WASH_NEW_SWC_sim_changes_test = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_LOCAL_WASH_NEW_SWC_sim_changes_test.length) {
					if (length < 1024 && commonByteArray_LOCAL_WASH_NEW_SWC_sim_changes_test.length == 0) {
						commonByteArray_LOCAL_WASH_NEW_SWC_sim_changes_test = new byte[1024];
					} else {
						commonByteArray_LOCAL_WASH_NEW_SWC_sim_changes_test = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_LOCAL_WASH_NEW_SWC_sim_changes_test, 0, length);
				strReturn = new String(commonByteArray_LOCAL_WASH_NEW_SWC_sim_changes_test, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_LOCAL_WASH_NEW_SWC_sim_changes_test) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tMysqlInput_3 = routines.system.PasswordEncryptUtil
						.decryptPassword("ENC:[eg3ye5bgW9AESWODLWXCJGeHYLcvHVOt42QYActrbr98HdSs]");

				String dbPwd_tMysqlInput_3 = decryptedPassword_tMysqlInput_3;

				String url_tMysqlInput_3 = "jdbc:mysql://" + "localhost" + ":" + "3306" + "/" + "WAReporting" + "?"
				...
				String dbquery_tMysqlInput_3 = "select last_etl_time \nfrom ETL_info_table \nwhere table_id = 10";
				...
				try {
					...
					while (rs_tMysqlInput_3.next()) {
						...
						if (colQtyInRs_tMysqlInput_3 < 1) {
							...
							row2.last_etl_time = routines.system.JDBCUtil.getString(rs_tMysqlInput_3, 1, false);
						}
...
// output_row.name = row2.name.toLowerCase();
	...
	public static class updated_flw_msisdn_recordsStruct
			implements routines.system.IPersistableRow<updated_flw_msisdn_recordsStruct> {
		final static byte[] commonByteArrayLock_LOCAL_WASH_NEW_SWC_sim_changes_test = new byte[0];
		static byte[] commonByteArray_LOCAL_WASH_NEW_SWC_sim_changes_test = new byte[0];
		...
		public Long new_msisdn;

		public Long getNew_msisdn() {
			return this.new_msisdn;
		}

		public java.util.Date transition_time;

		public java.util.Date getTransition_time() {
		...
		public boolean equals(Object obj) {
			...
			if (getClass() != obj.getClass())
				return false;
			final updated_flw_msisdn_recordsStruct other = (updated_flw_msisdn_recordsStruct) obj;
		...
		public void copyDataTo(updated_flw_msisdn_recordsStruct other) {

			other.id = this.id;
			other.flw_id = this.flw_id;
			other.old_msisdn = this.old_msisdn;
			other.new_msisdn = this.new_msisdn;
			other.transition_time = this.transition_time;

		}

		public void copyKeysDataTo(updated_flw_msisdn_recordsStruct other) {
			...
			synchronized (commonByteArrayLock_LOCAL_WASH_NEW_SWC_sim_changes_test) {
				...
				try {
					...
					if (length == -1) {
						...
						this.new_msisdn = dis.readLong();
					}

					this.transition_time = readDate(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				if (this.new_msisdn == null) {
					dos.writeByte(-1);
				} else {
					dos.writeByte(0);
					dos.writeLong(this.new_msisdn);
				}

				// java.util.Date

				writeDate(this.transition_time, dos);
		...
		public String toString() {
			...
			sb.append(",new_msisdn=" + String.valueOf(new_msisdn));
			sb.append(",transition_time=" + String.valueOf(transition_time));
			sb.append("]");
		 ...
		 * Compare keys
		 */
		public int compareTo(updated_flw_msisdn_recordsStruct other) {
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		...
		public Long oldCallingNumber;

		public Long getOldCallingNumber() {
			return this.oldCallingNumber;
		}

		public Long newCallingNumber;

		public Long getNewCallingNumber() {
			return this.newCallingNumber;
		}
			...
			synchronized (commonByteArrayLock_LOCAL_WASH_NEW_SWC_sim_changes_test) {
				...
				try {
					...
					if (length == -1) {
						...
						this.flwId = dis.readLong();
					}

					length = dis.readByte();
					if (length == -1) {
						this.oldCallingNumber = null;
					} else {
						this.oldCallingNumber = dis.readLong();
					}

					length = dis.readByte();
					if (length == -1) {
						this.newCallingNumber = null;
					} else {
						this.newCallingNumber = dis.readLong();
					}
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				if (this.flwId == null) {
					...
					dos.writeByte(0);
					dos.writeLong(this.flwId);
				}

				// Long

				if (this.oldCallingNumber == null) {
					dos.writeByte(-1);
				} else {
					dos.writeByte(0);
					dos.writeLong(this.oldCallingNumber);
				}

				// Long

				if (this.newCallingNumber == null) {
					dos.writeByte(-1);
				} else {
					dos.writeByte(0);
					dos.writeLong(this.newCallingNumber);
				}
		...
		public String toString() {
			...
			sb.append("[");
			sb.append("id=" + String.valueOf(id));
			sb.append(",flwId=" + String.valueOf(flwId));
			sb.append(",oldCallingNumber=" + String.valueOf(oldCallingNumber));
			sb.append(",newCallingNumber=" + String.valueOf(newCallingNumber));
	...
	public void tMysqlInput_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				updated_flw_msisdn_recordsStruct updated_flw_msisdn_records = new updated_flw_msisdn_recordsStruct();
				...
				if (execStat) {
					runStat.updateStatOnConnection(resourceMap, iterateId, 0, 0, "updated_flw_msisdn_records");
				}
				...
				String tableName_tMysqlOutput_1 = "swc_sim_changes";
				...
				final String decryptedPassword_tMysqlOutput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("ENC:[aolp8B2ub077P+FJnbC8O8BUNo4wUm5+Q+wkQkNY4ULGbC8u]");
				...
				java.sql.PreparedStatement pstmt_tMysqlOutput_1 = conn_tMysqlOutput_1
						.prepareStatement("SELECT COUNT(1) FROM `" + "swc_sim_changes" + "` WHERE `id` = ?");
				resourceMap.put("pstmt_tMysqlOutput_1", pstmt_tMysqlOutput_1);
				String insert_tMysqlOutput_1 = "INSERT INTO `" + "swc_sim_changes"
						+ "` (`id`,`flw_id`,`old_msisdn`,`new_msisdn`,`transition_time`) VALUES (?,?,?,?,?)";
				...
				String update_tMysqlOutput_1 = "UPDATE `" + "swc_sim_changes"
						+ "` SET `flw_id` = ?,`old_msisdn` = ?,`new_msisdn` = ?,`transition_time` = ? WHERE `id` = ?";
...
// # Outputs initialization
				updated_flw_msisdn_recordsStruct updated_flw_msisdn_records_tmp = new updated_flw_msisdn_recordsStruct();
// ###############################
				...
				final String decryptedPassword_tMysqlInput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("ENC:[gQr8gbBFB1qnBjn7QhR5PLwpD47wOrsVfOf1jqcQ1WkcoBEa]");
				...
				String dbquery_tMysqlInput_1 = "select id, swcId, oldCallingNumber, newCallingNumber, modificationDate from wa_contactNumber_audit\nwhere modificationDa"
						+ "te > '" + (String) globalMap.get("lastEtlTime") + "'";
				...
				try {
					...
					while (rs_tMysqlInput_1.next()) {
						...
						if (colQtyInRs_tMysqlInput_1 < 2) {
							...
							row1.flwId = rs_tMysqlInput_1.getLong(2);
							if (rs_tMysqlInput_1.wasNull()) {
								row1.flwId = null;
							}
						}
						if (colQtyInRs_tMysqlInput_1 < 3) {
							row1.oldCallingNumber = null;
						} else {

							row1.oldCallingNumber = rs_tMysqlInput_1.getLong(3);
							if (rs_tMysqlInput_1.wasNull()) {
								row1.oldCallingNumber = null;
							}
						}
						if (colQtyInRs_tMysqlInput_1 < 4) {
							row1.newCallingNumber = null;
						} else {

							row1.newCallingNumber = rs_tMysqlInput_1.getLong(4);
							if (rs_tMysqlInput_1.wasNull()) {
								row1.newCallingNumber = null;
							}
						}
						if (colQtyInRs_tMysqlInput_1 < 5) {
							...
							if (rs_tMysqlInput_1.getString(5) != null) {
								...
								if (!("0000-00-00").equals(dateString_tMysqlInput_1)
									...
									row1.modificationDate = rs_tMysqlInput_1.getTimestamp(5);
								} else {
						...
						{ // start of Var scope
							...
							updated_flw_msisdn_records = null;

// # Output table : 'updated_flw_msisdn_records'
							updated_flw_msisdn_records_tmp.id = row1.id;
							updated_flw_msisdn_records_tmp.flw_id = row1.flwId;
							updated_flw_msisdn_records_tmp.old_msisdn = row1.oldCallingNumber;
							updated_flw_msisdn_records_tmp.new_msisdn = row1.newCallingNumber;
							updated_flw_msisdn_records_tmp.transition_time = row1.modificationDate;
							updated_flw_msisdn_records = updated_flw_msisdn_records_tmp;
// ###############################
						 ...
						 */
// Start of branch "updated_flw_msisdn_records"
						if (updated_flw_msisdn_records != null) {
							...
							if (execStat) {
								runStat.updateStatOnConnection(iterateId, 1, 1, "updated_flw_msisdn_records");
							}

							whetherReject_tMysqlOutput_1 = false;
							pstmt_tMysqlOutput_1.setInt(1, updated_flw_msisdn_records.id);
							...
							if (checkCount_tMysqlOutput_1 > 0) {
								if (updated_flw_msisdn_records.flw_id == null) {
									pstmtUpdate_tMysqlOutput_1.setNull(1, java.sql.Types.INTEGER);
								} else {
									pstmtUpdate_tMysqlOutput_1.setLong(1, updated_flw_msisdn_records.flw_id);
								}

								if (updated_flw_msisdn_records.old_msisdn == null) {
									pstmtUpdate_tMysqlOutput_1.setNull(2, java.sql.Types.INTEGER);
								} else {
									pstmtUpdate_tMysqlOutput_1.setLong(2, updated_flw_msisdn_records.old_msisdn);
								}

								if (updated_flw_msisdn_records.new_msisdn == null) {
									pstmtUpdate_tMysqlOutput_1.setNull(3, java.sql.Types.INTEGER);
								} else {
									pstmtUpdate_tMysqlOutput_1.setLong(3, updated_flw_msisdn_records.new_msisdn);
								}

								if (updated_flw_msisdn_records.transition_time != null) {
									date_tMysqlOutput_1 = updated_flw_msisdn_records.transition_time.getTime();
									if (date_tMysqlOutput_1 < year1_tMysqlOutput_1
										...
										pstmtUpdate_tMysqlOutput_1.setTimestamp(4,
								...
								pstmtUpdate_tMysqlOutput_1.setInt(5 + count_tMysqlOutput_1,
										updated_flw_msisdn_records.id);
								...
								pstmtInsert_tMysqlOutput_1.setInt(1, updated_flw_msisdn_records.id);

								if (updated_flw_msisdn_records.flw_id == null) {
									pstmtInsert_tMysqlOutput_1.setNull(2, java.sql.Types.INTEGER);
								} else {
									pstmtInsert_tMysqlOutput_1.setLong(2, updated_flw_msisdn_records.flw_id);
								}

								if (updated_flw_msisdn_records.old_msisdn == null) {
									pstmtInsert_tMysqlOutput_1.setNull(3, java.sql.Types.INTEGER);
								} else {
									pstmtInsert_tMysqlOutput_1.setLong(3, updated_flw_msisdn_records.old_msisdn);
								}

								if (updated_flw_msisdn_records.new_msisdn == null) {
									pstmtInsert_tMysqlOutput_1.setNull(4, java.sql.Types.INTEGER);
								} else {
									pstmtInsert_tMysqlOutput_1.setLong(4, updated_flw_msisdn_records.new_msisdn);
								}
						...
						} // End of branch "updated_flw_msisdn_records"
...
// ###############################      
				...
				if (execStat) {
					runStat.updateStat(resourceMap, iterateId, 2, 0, "updated_flw_msisdn_records");
				}
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tMysqlRow_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("ENC:[qau/HP5MrIgYa5/vK3r+kki2n4pc8mqXanMW6d/gEpOdnofK]");
				...
				query_tMysqlRow_1 = "update ETL_info_table\nset ETL_info_table.last_etl_time = (select max(transition_time) from swc_sim_changes),\nETL_info_t"
						+ "able.record_count = (select count(*) from swc_sim_changes)\nwhere ETL_info_table.table_id = 10";
	...
	public static void main(String[] args) {
		final SWC_sim_changes_test SWC_sim_changes_testClass = new SWC_sim_changes_test();

		int exitCode = SWC_sim_changes_testClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = SWC_sim_changes_test.class.getClassLoader().getResourceAsStream(
					"local_wash_new/swc_sim_changes_test_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = SWC_sim_changes_test.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : SWC_sim_changes_test");
		}
...
/************************************************************************************************
 * 101479 characters generated by Talend Open Studio for Data Integration on the
 * 19 November, 2019 6:36:15 PM IST
 ************************************************************************************************/