...
package wash_odf_updated.wa_course_completion_cummulative_0_1;
...
/**
 * Job: WA_Course_Completion_Cummulative Purpose: WA_Course_Completion<br>
 * Description: WA_Course_Completion <br>
 * @author user@talend.com
 * @version 7.3.1.20191031_1204-M3
 * @status 
 */
public class WA_Course_Completion_Cummulative implements TalendJob {
...
public String getMotech_db_Server(){
	...
	private final String jobName = "WA_Course_Completion_Cummulative";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				WA_Course_Completion_Cummulative.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(WA_Course_Completion_Cummulative.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
    final static byte[] commonByteArrayLock_WASH_ODF_UPDATED_WA_Course_Completion_Cummulative = new byte[0];
    static byte[] commonByteArray_WASH_ODF_UPDATED_WA_Course_Completion_Cummulative = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_WASH_ODF_UPDATED_WA_Course_Completion_Cummulative.length) {
				if(length < 1024 && commonByteArray_WASH_ODF_UPDATED_WA_Course_Completion_Cummulative.length == 0) {
   					commonByteArray_WASH_ODF_UPDATED_WA_Course_Completion_Cummulative = new byte[1024];
				} else {
   					commonByteArray_WASH_ODF_UPDATED_WA_Course_Completion_Cummulative = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_WASH_ODF_UPDATED_WA_Course_Completion_Cummulative, 0, length);
			strReturn = new String(commonByteArray_WASH_ODF_UPDATED_WA_Course_Completion_Cummulative, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_WASH_ODF_UPDATED_WA_Course_Completion_Cummulative) {
 ...
 */
	...
	final String decryptedPassword_tMysqlInput_1 = routines.system.PasswordEncryptUtil.decryptPassword("ENC:[9mTZygX5aoZfT8h90+lFJjrZ/bSz/5paZLBCuxxYXPQpWo7S]");
...
public static class copyOfMa_course_completion_dataStruct implements routines.system.IPersistableRow<copyOfMa_course_completion_dataStruct> {
	...
	public void copyDataTo(copyOfMa_course_completion_dataStruct other) {
		...
		other.swc_id = this.swc_id;
	            ...
	            other.swc_name = this.swc_name;
	            other.creation_date = this.creation_date;
	            other.job_status = this.job_status;
	            other.first_completion = this.first_completion;
...
String tableName_tMysqlOutput_2 = "WA_course_completion_first";
...
java.sql.Connection conn_tMysqlOutput_2 = null;
	...
	final String decryptedPassword_tMysqlOutput_2 = routines.system.PasswordEncryptUtil.decryptPassword("ENC:[xDJVZjwwU2gZ2G4DdVvhPXHLhRJIq/BHUsUns9GTOjHV5LaX]");
...
int count_tMysqlOutput_2=0;
	    ...
	    String update_tMysqlOutput_2 = "UPDATE `" + "WA_course_completion_first" + "` SET `mobile_number` = ?,`state_id` = ?,`district_id` = ?,`panchayat_id` = ?,`block_id` = ?,`swc_name` = ?,`creation_date` = ?,`job_status` = ?,`first_completion` = ?,`sent_notification` = ?,`course_start_date` = ?,`courseId` = ? WHERE `swc_id` = ?";
	    ...
	    String insert_tMysqlOutput_2 = "INSERT INTO `" + "WA_course_completion_first" + "` (`swc_id`,`mobile_number`,`state_id`,`district_id`,`panchayat_id`,`block_id`,`swc_name`,`creation_date`,`job_status`,`first_completion`,`sent_notification`,`course_start_date`,`courseId`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
 ...
 */
	...
	final String decryptedPassword_tMysqlInput_3 = routines.system.PasswordEncryptUtil.decryptPassword("ENC:[7rMXYybhqMlJIQLjVR4YJZ2b6Fik7WmUQVcfPKua/cRv6Vnx]");
...
+"       flw.panchayat_id,\n            flw.block_id,\n            flw.name,\n            flw.creation_date,\n            flw."
+"job_status,\n			wcds.course_start_date,\n			mcc1.latest as first_completion,\n            mcc.sent_notification,\n          "
+"  mcc.courseId\n     FROM\n        WA_course_completion mcc\n   INNER JOIN swachchagrahi flw ON flw.swc_id = mcc.swc_id \n  "
+" inner  join (select  swc_id, min(creation_date) as latest, courseId from WA_course_completion where has_passed= true gr"
+"oup by  swc_id, courseId ) as mcc1 \n   on mcc1.swc_id= mcc.swc_id and mcc1.latest= mcc.creation_date and mcc1.courseId ="
+" mcc.courseId\n   left join (select  swc_id, min(start_time) as course_start_date, courseId from WA_call_detail_measure g"
+"roup by swc_id, courseId ) as wcds \n   on wcds.swc_id= mcc.swc_id and wcds.courseId = mcc.courseId\nWHERE\n        mcc.has"
+"_passed = 1\n            AND flw.job_status = 'ACTIVE'"
;
            ...
            row4.swc_id = rs_tMysqlInput_3.getLong(1);
            if(rs_tMysqlInput_3.wasNull()){
                    throw new RuntimeException("Null value in non-Nullable column");
            }
		                    }
							if(colQtyInRs_tMysqlInput_3 < 2) {
								row4.mobile_number = null;
							} else {
		                          
            row4.mobile_number = rs_tMysqlInput_3.getLong(2);
            if(rs_tMysqlInput_3.wasNull()){
                    row4.mobile_number = null;
            }
		                    }
            ...
            row4.state_id = rs_tMysqlInput_3.getInt(3);
            if(rs_tMysqlInput_3.wasNull()){
                    row4.state_id = null;
            }
		                    }
            ...
            row4.district_id = rs_tMysqlInput_3.getInt(4);
            if(rs_tMysqlInput_3.wasNull()){
                    row4.district_id = null;
            }
		                    }
            ...
            row4.panchayat_id = rs_tMysqlInput_3.getInt(5);
            if(rs_tMysqlInput_3.wasNull()){
                    row4.panchayat_id = null;
            }
		                    }
            ...
            row4.block_id = rs_tMysqlInput_3.getLong(6);
            if(rs_tMysqlInput_3.wasNull()){
                    row4.block_id = null;
            }
		                    }
        	...
        	row4.name = routines.system.JDBCUtil.getString(rs_tMysqlInput_3, 7, false);
		                    }
							if(colQtyInRs_tMysqlInput_3 < 8) {
								row4.creation_date = null;
							} else {
	                         		
        	row4.creation_date = routines.system.JDBCUtil.getString(rs_tMysqlInput_3, 8, false);
		                    }
							if(colQtyInRs_tMysqlInput_3 < 9) {
								row4.job_status = null;
							} else {
	                         		
        	row4.job_status = routines.system.JDBCUtil.getString(rs_tMysqlInput_3, 9, false);
		                    }
							if(colQtyInRs_tMysqlInput_3 < 10) {
								row4.course_start_date = null;
							} else {
	                         		
        	row4.course_start_date = routines.system.JDBCUtil.getString(rs_tMysqlInput_3, 10, false);
		                    }
							if(colQtyInRs_tMysqlInput_3 < 11) {
								row4.first_completion = null;
							} else {
	                         		
        	row4.first_completion = routines.system.JDBCUtil.getString(rs_tMysqlInput_3, 11, false);
		                    }
							if(colQtyInRs_tMysqlInput_3 < 12) {
								row4.sent_notification = null;
							} else {
	                         		
            row4.sent_notification = rs_tMysqlInput_3.getBoolean(12);
            if(rs_tMysqlInput_3.wasNull()){
                    row4.sent_notification = null;
            }
		                    }
            ...
            row4.courseId = rs_tMysqlInput_3.getInt(13);
            if(rs_tMysqlInput_3.wasNull()){
                    throw new RuntimeException("Null value in non-Nullable column");
            }
		                    }
...
copyOfMa_course_completion_data_tmp.swc_id = row4.swc_id ;
copyOfMa_course_completion_data_tmp.mobile_number = row4.mobile_number ;
copyOfMa_course_completion_data_tmp.state_id = row4.state_id ;
copyOfMa_course_completion_data_tmp.district_id = row4.district_id ;
copyOfMa_course_completion_data_tmp.panchayat_id = row4.panchayat_id ;
copyOfMa_course_completion_data_tmp.block_id = row4.block_id ;
copyOfMa_course_completion_data_tmp.swc_name = row4.name ;
copyOfMa_course_completion_data_tmp.creation_date = row4.creation_date ;
copyOfMa_course_completion_data_tmp.job_status = row4.job_status ;
copyOfMa_course_completion_data_tmp.first_completion = row4.first_completion ;
copyOfMa_course_completion_data_tmp.sent_notification = row4.sent_notification ;
	...
	currentComponent="tMysqlOutput_2";
        ...
        whetherReject_tMysqlOutput_2 = false;
            int updateFlag_tMysqlOutput_2=0;
                    if(copyOfMa_course_completion_data.mobile_number == null) {
pstmtUpdate_tMysqlOutput_2.setNull(1, java.sql.Types.INTEGER);
} else {pstmtUpdate_tMysqlOutput_2.setLong(1, copyOfMa_course_completion_data.mobile_number);
}

                    if(copyOfMa_course_completion_data.state_id == null) {
pstmtUpdate_tMysqlOutput_2.setNull(2, java.sql.Types.INTEGER);
} else {pstmtUpdate_tMysqlOutput_2.setInt(2, copyOfMa_course_completion_data.state_id);
}

                    if(copyOfMa_course_completion_data.district_id == null) {
pstmtUpdate_tMysqlOutput_2.setNull(3, java.sql.Types.INTEGER);
} else {pstmtUpdate_tMysqlOutput_2.setInt(3, copyOfMa_course_completion_data.district_id);
}

                    if(copyOfMa_course_completion_data.panchayat_id == null) {
pstmtUpdate_tMysqlOutput_2.setNull(4, java.sql.Types.INTEGER);
} else {pstmtUpdate_tMysqlOutput_2.setInt(4, copyOfMa_course_completion_data.panchayat_id);
}

                    if(copyOfMa_course_completion_data.block_id == null) {
pstmtUpdate_tMysqlOutput_2.setNull(5, java.sql.Types.INTEGER);
} else {pstmtUpdate_tMysqlOutput_2.setLong(5, copyOfMa_course_completion_data.block_id);
}

                    if(copyOfMa_course_completion_data.swc_name == null) {
pstmtUpdate_tMysqlOutput_2.setNull(6, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tMysqlOutput_2.setString(6, copyOfMa_course_completion_data.swc_name);
}

                    if(copyOfMa_course_completion_data.creation_date == null) {
pstmtUpdate_tMysqlOutput_2.setNull(7, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tMysqlOutput_2.setString(7, copyOfMa_course_completion_data.creation_date);
}

                    if(copyOfMa_course_completion_data.job_status == null) {
pstmtUpdate_tMysqlOutput_2.setNull(8, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tMysqlOutput_2.setString(8, copyOfMa_course_completion_data.job_status);
}

                    if(copyOfMa_course_completion_data.first_completion == null) {
pstmtUpdate_tMysqlOutput_2.setNull(9, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tMysqlOutput_2.setString(9, copyOfMa_course_completion_data.first_completion);
}

                    if(copyOfMa_course_completion_data.sent_notification == null) {
pstmtUpdate_tMysqlOutput_2.setNull(10, java.sql.Types.BOOLEAN);
} else {pstmtUpdate_tMysqlOutput_2.setBoolean(10, copyOfMa_course_completion_data.sent_notification);
}

                    if(copyOfMa_course_completion_data.course_start_date == null) {
pstmtUpdate_tMysqlOutput_2.setNull(11, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tMysqlOutput_2.setString(11, copyOfMa_course_completion_data.course_start_date);
}

                    pstmtUpdate_tMysqlOutput_2.setInt(12, copyOfMa_course_completion_data.courseId);


                    pstmtUpdate_tMysqlOutput_2.setLong(13 + count_tMysqlOutput_2, copyOfMa_course_completion_data.swc_id);


            try {
                updateFlag_tMysqlOutput_2=pstmtUpdate_tMysqlOutput_2.executeUpdate();
                updatedCount_tMysqlOutput_2 = updatedCount_tMysqlOutput_2+updateFlag_tMysqlOutput_2;

            if(updateFlag_tMysqlOutput_2 == 0) {

                        pstmtInsert_tMysqlOutput_2.setLong(1, copyOfMa_course_completion_data.swc_id);

                        if(copyOfMa_course_completion_data.mobile_number == null) {
pstmtInsert_tMysqlOutput_2.setNull(2, java.sql.Types.INTEGER);
} else {pstmtInsert_tMysqlOutput_2.setLong(2, copyOfMa_course_completion_data.mobile_number);
}

                        if(copyOfMa_course_completion_data.state_id == null) {
pstmtInsert_tMysqlOutput_2.setNull(3, java.sql.Types.INTEGER);
} else {pstmtInsert_tMysqlOutput_2.setInt(3, copyOfMa_course_completion_data.state_id);
}

                        if(copyOfMa_course_completion_data.district_id == null) {
pstmtInsert_tMysqlOutput_2.setNull(4, java.sql.Types.INTEGER);
} else {pstmtInsert_tMysqlOutput_2.setInt(4, copyOfMa_course_completion_data.district_id);
}

                        if(copyOfMa_course_completion_data.panchayat_id == null) {
pstmtInsert_tMysqlOutput_2.setNull(5, java.sql.Types.INTEGER);
} else {pstmtInsert_tMysqlOutput_2.setInt(5, copyOfMa_course_completion_data.panchayat_id);
}

                        if(copyOfMa_course_completion_data.block_id == null) {
pstmtInsert_tMysqlOutput_2.setNull(6, java.sql.Types.INTEGER);
} else {pstmtInsert_tMysqlOutput_2.setLong(6, copyOfMa_course_completion_data.block_id);
}

                        if(copyOfMa_course_completion_data.swc_name == null) {
pstmtInsert_tMysqlOutput_2.setNull(7, java.sql.Types.VARCHAR);
} else {pstmtInsert_tMysqlOutput_2.setString(7, copyOfMa_course_completion_data.swc_name);
}

                        if(copyOfMa_course_completion_data.creation_date == null) {
pstmtInsert_tMysqlOutput_2.setNull(8, java.sql.Types.VARCHAR);
} else {pstmtInsert_tMysqlOutput_2.setString(8, copyOfMa_course_completion_data.creation_date);
}

                        if(copyOfMa_course_completion_data.job_status == null) {
pstmtInsert_tMysqlOutput_2.setNull(9, java.sql.Types.VARCHAR);
} else {pstmtInsert_tMysqlOutput_2.setString(9, copyOfMa_course_completion_data.job_status);
}

                        if(copyOfMa_course_completion_data.first_completion == null) {
pstmtInsert_tMysqlOutput_2.setNull(10, java.sql.Types.VARCHAR);
} else {pstmtInsert_tMysqlOutput_2.setString(10, copyOfMa_course_completion_data.first_completion);
}

                        if(copyOfMa_course_completion_data.sent_notification == null) {
pstmtInsert_tMysqlOutput_2.setNull(11, java.sql.Types.BOOLEAN);
} else {pstmtInsert_tMysqlOutput_2.setBoolean(11, copyOfMa_course_completion_data.sent_notification);
}

                        if(copyOfMa_course_completion_data.course_start_date == null) {
pstmtInsert_tMysqlOutput_2.setNull(12, java.sql.Types.VARCHAR);
} else {pstmtInsert_tMysqlOutput_2.setString(12, copyOfMa_course_completion_data.course_start_date);
}
...
public void tMysqlRow_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 final boolean execStat = this.execStat;
	...
	final String decryptedPassword_tMysqlRow_1 = routines.system.PasswordEncryptUtil.decryptPassword("ENC:[LSLZSscYSFqmmJMCGZFXhGtBKuWSzPf+EsyetKIINOlwbnhA]");
...
query_tMysqlRow_1 = "UPDATE ETL_info_table SET ETL_info_table.last_etl_time = (SELECT MAX(first_completion)FROM \n WA_course_completion_first"
+"),\nETL_info_table.record_count=(select count(*) from WA_course_completion_first)\nWHERE ETL_info_table.table_id = 16";
 ...
 */
    ...
    public static void main(String[] args){
        final WA_Course_Completion_Cummulative WA_Course_Completion_CummulativeClass = new WA_Course_Completion_Cummulative();

        int exitCode = WA_Course_Completion_CummulativeClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = WA_Course_Completion_Cummulative.class.getClassLoader().getResourceAsStream("wash_odf_updated/wa_course_completion_cummulative_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = WA_Course_Completion_Cummulative.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : WA_Course_Completion_Cummulative");
        }
...
/************************************************************************************************
 *     114900 characters generated by Talend Open Studio for Data Integration 
